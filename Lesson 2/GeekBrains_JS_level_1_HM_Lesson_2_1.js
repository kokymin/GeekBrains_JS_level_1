...
//1.3 результат: 4. Т.к. префиксная форма возвращает новое значение, которое можно сразу испльзовать ++а = 2.
//Также инкремент имеет больший приоритет перед сложением, в данном примере выолняется первым.
c = 2+ ++a;
console.log(c);

//1.4 результат: 4. Т.к. в 1.2 b было увеличено на 1 (b++), префиксная форма вернула это значение.
//Также инкремент имеет больший приоритет перед сложением, в данном примере выолняется первым.
d = 2 + b++;
console.log(d);
